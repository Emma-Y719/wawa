...
Page({
  ...
  getsuggest: function (e) {
    ...
    qqmapsdk.getSuggestion({
      ...
      success: function (res) {//搜索成功后的回调
        //console.log(res);
        var sug = [];
        console.log("latitude:", res.data[0].location.lat,"longitude:", res.data[0].location.lng);
  ...
  selectedOk: function () {
    ...
    wx.setStorageSync("loc",this.data.centerData.title)
    wx.setStorageSync("log",this.data.centerData.longitude)
    wx.setStorageSync("lat",this.data.centerData.latitude)
    let pages = getCurrentPages(); //获取当前页面js里面的pages里的所有信息。
    let prevPage = pages[pages.length - 2]; 
    console.log(this.data.centerData)
    prevPage.setData({
      loc: this.data.centerData.title,
      log: this.data.centerData.longitude,
      lat: this.data.centerData.latitude,
    })
    wx.navigateBack({
      delta: 1
    })
